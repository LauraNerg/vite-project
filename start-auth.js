...
loginUser.addEventListener('click', async (evt) => {
  ...
  const url = 'https://laura.northeurope.cloudapp.azure.com/api/auth/login';
  ...
  fetchData(url, options).then((data) => {
    ...
    // jos on token, console loggaa, että kaikki hyvin. Jos ei niin console.log ei ollut
    ...
    if (data.token == undefined) {
      ...
      alert('Siirrytään päiväkirjasivulle');
      setTimeout(function () {
...
function logResponse(codeblock, text) {
  ...
  console.log('Homma meni onnistuneesti eteenpäin');
}
